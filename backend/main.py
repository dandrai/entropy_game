...
async def session_stats(session_id: str):
    ...
    return {
        ...
        "entropy": _mean_log2_g(words_data),
...
async def get_leaderboard(corpus_id: Optional[str] = None):
    ...
    for row in wr_rows:
        ...
        buckets[key].append({"g_value": row["g_value"]})
...
async def _load_words_data(db: aiosqlite.Connection, session_id: str) -> list[dict]:
    db.row_factory = aiosqlite.Row
    wr_rows = await (await db.execute(
        "SELECT g_value FROM word_results WHERE session_id = ?",
        (session_id,),
    )).fetchall()
    return [{"g_value": r["g_value"]} for r in wr_rows]


async def _session_score(db: aiosqlite.Connection, session_id: str) -> float:
    words_data = await _load_words_data(db, session_id)
    return _mean_log2_g(words_data)


def _mean_log2_g(words_data: list[dict]) -> float:
    """Mean of log₂(G) — original Kolmogorov/Shannon entropy estimator."""
    n = len(words_data)
    if n == 0:
        return 0.0
    return round(sum(math.log2(w["g_value"]) for w in words_data) / n, 4)
